// Mama's Kitchen — Stripe Checkout (Netlify Function).
// Activates once STRIPE_SECRET_KEY is set in Netlify env. Until then the site
// falls back to emailing the order. When a logged-in customer authorizes
// recurring billing, the card is saved on file (off_session) for the weekly
// auto-charge handled by weekly-billing.js.
...
exports.handler = async (event) => {
  ...
  if (!order || !Array.isArray(order.meals)) return { statusCode: 400, body: JSON.stringify({ error: 'Invalid order' }) };
  ...
  if (mealCount === 0 || subtotal <= 0) return { statusCode: 400, body: JSON.stringify({ error: 'Empty order' }) };
