// Mama's Kitchen — weekly recurring billing (Netlify Scheduled Function).
// Schedule set in netlify.toml (Sunday ~11 PM ET / Mon 04:00 UTC).
//
// SAFETY: This NEVER charges anyone unless ALL of these are true:
//   1. env BILLING_ENABLED === 'true'   (master off switch — default OFF)
//   2. the customer ticked "authorize weekly charges" (billing_authorized)
//   3. their recurring order is active and not paused
//   4. they have a saved card (stripe_customer_id + payment_method)
//   5. they were not already charged in the last 4 days (dedupe guard)
//
// Requires env: STRIPE_SECRET_KEY, SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY,
//               BILLING_ENABLED=true, OWNER_EMAIL (optional, defaults below).
...
exports.handler = async () => {
  if (process.env.BILLING_ENABLED !== 'true') { console.log('billing disabled'); return { statusCode: 200, body: 'disabled' }; }
  const key = process.env.STRIPE_SECRET_KEY;
  if (!key || !process.env.SUPABASE_URL || !process.env.SUPABASE_SERVICE_ROLE_KEY) { return { statusCode: 200, body: 'not configured' }; }
  ...
  const { data: recs, error } = await db.from('recurring_orders')
    .select('customer_id, config, active, paused, billing_authorized, last_charged_at, profiles!inner(stripe_customer_id, stripe_payment_method_id, full_name, email, phone)')
    .eq('active', true).eq('paused', false).eq('billing_authorized', true);
  if (error) { console.error(error); return { statusCode: 500, body: 'query error' }; }
  ...
  for (const r of (recs || [])) {
    ...
    try {
      ...
      if (oid) {
        ...
        const bulkRows = (cfg.bulk || []).map(b => { const arr = b.cat === 'protein' ? BULK_PROTEIN : (b.cat === 'carb' ? BULK_CARB : BULK_VEG);
          const unit = b.cat === 'protein' ? BULK_PROTEIN[b.idx] : (b.cat === 'carb' ? BULK_CARB : BULK_VEG);
  ...
  console.log(`billing run: charged=${charged} failed=${failed} skipped=${skipped}`);
  return { statusCode: 200, body: JSON.stringify({ charged, failed, skipped }) };
};
